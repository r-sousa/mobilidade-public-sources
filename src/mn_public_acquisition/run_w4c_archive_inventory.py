...
import yaml
from .extract_w4c_archive_inventory import run
...
def main() -> None:
    ...
    if not read_token or not write_token: raise SystemExit('MN_PRIVATE_READ_AND_SINK_TOKEN_REQUIRED')
    try: result=run(cfg,read_token,write_token)
    except Exception as exc:
        result={'schema_version':'4.0.1','complete':False,'error':str(exc)}
        args.out.parent.mkdir(parents=True,exist_ok=True); args.out.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(json.dumps(result,ensure_ascii=False,indent=2)); raise
