...
| `mn26-w1-jcyl-source.yml`, `mn26-w4-jcyl-source.yml` | `opendatasoft` / static HTTP / file-series | yes while canonical rights unresolved | F181 live pagination+recomposition proven; F31-F34 preserved-first trade family registered | Retire only exact products after preserved packages are consumed first |
...
| `mn26-cnmc-rail-source.yml` | static/API or HTML-discovery family | yes | Generic file/HTML route proven; CNMC product-specific route not yet pinned | Keep until exact product spec is validated |
| `mn26-dgt-source.yml`, `mn26-dgt-cos-source.yml` | `arcgis_feature_service` | yes | F66 live complete-query + recomposition proven | Migrate exact layers after preservation review |
